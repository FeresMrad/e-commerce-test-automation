...
test('TC-14: Place Order: Register while Checkout', async ({ page }) => {
  ...
  // 7. Proceed To Checkout (modal asks to login)
  await page.getByText('Proceed To Checkout').click();

  // 8. Click Register / Login in the modal
  await page.getByRole('link', { name: /register \/ login/i }).click();

  // 9. Signup with newUser (full register flow)
  ...
  // 12-13. Cart -> Proceed To Checkout
  await page.getByRole('link', { name: 'Cart' }).first().click();
  await page.getByText('Proceed To Checkout').click();

  // 14. Address Details + Review Your Order are visible
